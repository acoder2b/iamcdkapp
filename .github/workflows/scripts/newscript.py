...
def create_yaml_file(roles_data, account_id):
    """
    Create a YAML file from IAM role details.
    """
    yaml_content = []
    ...
    try:
        ...
        logging.info(f"YAML file {yaml_file_name} created successfully with {len(roles_data)} roles.")

    except Exception as error:
        logging.error(f"Error writing YAML file: {error}")
...
def main():
    ...
    # Fetch role details and create YAML
    roles_data = []
    ...
    if roles_data:
        ...
        create_yaml_file(roles_data, account_id)
    else:
